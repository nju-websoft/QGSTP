...
public abstract class GraphBase{
   ...
   /* public GraphBase(DogReader d1, String graphName){
    ...
    //read from stardog
    /*protected void readGraph(DogReader d1, String graphName){
        this.graphName = graphName;
        Connection connection = d1.getConnection();
        try {
            PrintWriter pt = new PrintWriter(new File("C:\\kgdata\\keywords.txt"));
            SelectQuery aQuery = connection.select("select * where { ?s ?p ?o }");
            SelectQueryResult r = aQuery.execute();

            while (r.hasNext()){
                BindingSet next = r.next(); //real triple

                //if subject is literal
                if (!next.resource("s").isPresent())
                    continue;
                int u = tryAddNode(Objects.requireNonNull(next.get("s")).toString());
                if (next.iri("p").isPresent()){
                    Optional<IRI> predi = next.iri("p");

                    //type is used to calculate pairwise distance
                    if ("type".equals(predi.get().localName())){
                        String nodeType = Objects.requireNonNull(next.get("o")).toString();
                        if (nodeType.equals("http://www.w3.org/2002/07/owl#NamedIndividual")) {
                            short typeNum = tryAddType(nodeType);
                            typeList.get(u).add(typeNum);
                            continue;
                        }
                        int v = tryAddNode(nodeType);
                        int uv = tryAddEdge(Objects.requireNonNull(next.get("o")).toString());
                    ...
                    //label
                    if ("label".equals(predi.get().localName())){
                        String nodeLabel = Objects.requireNonNull(next.get("o")).toString();
                        if (!nodeLabel.endsWith("@en"))
                            continue;
                        nodeLabel = nodeLabel.substring(1, nodeLabel.length() - 4);
                        labelList.get(u).add(nodeLabel);
                        continue;
                    }
                    //real graphs, where two endpoints are both nodes
                    if (next.resource("s").isPresent() && next.resource("p").isPresent()){
                        //add nodes and edges to graph
                        int v = tryAddNode(Objects.requireNonNull(next.get("o")).toString());
                        int uv = tryAddEdge(Objects.requireNonNull(next.get("p")).toString());
                        graph.get(u).add(new HopVDE(v, uv));
                        graph.get(v).add(new HopVDE(u, uv));
                    }
                }
            ...
            d1.releaseConnection(connection);
        }
        graphPrepare();
    }
*/
     ...
     * @param graphName the graph name in the database
     */
